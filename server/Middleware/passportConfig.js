    ...
    async (accessToken, refreshToken, profile, done) => {
      ...
      try {
        const [existingUser] = await connection.query(
          "SELECT * FROM techcoach_lite.techcoach_task WHERE email=?",
          [profile.email]
        );
        if (!existingUser || existingUser.length === 0) {
          // Check if existingUser is undefined or empty array
          // Insert the new user
          const user = await connection.query(
            "INSERT INTO techcoach_lite.techcoach_task (displayname, email) VALUES (?, ?) RETURNING* ",
